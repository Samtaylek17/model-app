...
exports.resetPassword = catchAsync(async (req, res, next) => {
	...
	// 2) If token has not expired, and user is present, set the new password
	if (!user) {
...
exports.verifyEmail = catchAsync(async (req, res, next) => {
	...
	// 3) Send toke to user's email
	try {
		let emailURL = '';
		if (req.body.requester === 'client') {
			emailURL = `${req.protocol}://${req.get(
				'host'
		...
		await new Email(user, emailURL).sendVerifyEmail();
...
exports.confirmEmailToken = catchAsync(async (req, res, next) => {
	// 1) Get user based on the token
	const hashedToken = crypto
		.createHash('sha256')
		.update(req.params.token)
